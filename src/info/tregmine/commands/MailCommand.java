		...
		if(args[0].contains("send")){
			...
			try (IContext ctx = tregmine.createContext()) {
		        ...
		        if(success){
		        	...
		        	player.sendStringMessage("%internal%" + ChatColor.RED + "Message failed, check the username and send again.");
		        }
			...
			if(getArgs(args) < 2){
				...
				try (IContext ctx = tregmine.createContext()) {
			        ...
			        if(success){
			        	...
			        	player.sendStringMessage("%internal%" + ChatColor.RED + "Delete failed, check the id and try again.");
			        }
			    } catch (DAOException e) {
			        throw new RuntimeException(e);
			...
			try (IContext ctx = tregmine.createContext()) {
		        ...
		        if(messages.size() == 0){
		        	...
		        	player.sendStringMessage("%internal%" + ChatColor.AQUA + "You have " + messages.size() + " message.");
		        }else{
		        	player.sendStringMessage("%internal%" + ChatColor.AQUA + "You have " + messages.size() + " messages.");
		        }
		    } catch (DAOException e) {
		        throw new RuntimeException(e);
		    }
			...
			if(player.getIsStaff()){
				if(getArgs(args) < 2){
					...
					try (IContext ctx = tregmine.createContext()) {
				        ...
				        player.sendStringMessage("%internal%" + ChatColor.AQUA + args[1] + " has received " + amount + " " + suffix + " during their time on Tregmine.");
				    } catch (DAOException e) {
				        throw new RuntimeException(e);
				    }
				}
				...
				try (IContext ctx = tregmine.createContext()) {
			        ...
			        player.sendStringMessage("%internal%" + ChatColor.AQUA + "You have received " + amount + " " + suffix + " during your time on Tregmine.");
			    } catch (DAOException e) {
			        throw new RuntimeException(e);
			    }
			}
			...
			try (IContext ctx = tregmine.createContext()) {
		        ...
		        for(String[] message : messages){
		        	player.sendStringMessage("%internal%" + ChatColor.AQUA + "You have a message from " + message[0] + " [ID " + message[4] + "]");
