...
public class MailCommand extends AbstractCommand {
	...
	public boolean handlePlayer(TregminePlayer player, String[] args) {
		...
		if (args[0].contains("send")) {
			...
			try (IContext ctx = tregmine.createContext()) {
				...
				if (success) {
					player.sendStringMessage("%internal%" + ChatColor.AQUA + "Message sent successfully! :)");
				} else {
					player.sendStringMessage(
							"%internal%" + ChatColor.RED + "Message failed, check the username and send again.");
				}
			...
			if (getArgs(args) < 2) {
				player.sendStringMessage("%internal%" + ChatColor.RED + "You must specify what to delete!");
			} else {
				try (IContext ctx = tregmine.createContext()) {
					IMailDAO mail = ctx.getMailDAO();
					...
					if (success) {
						player.sendStringMessage("%internal%" + ChatColor.AQUA + "Message deleted successfully! :)");
					} else {
						player.sendStringMessage(
								"%internal%" + ChatColor.RED + "Delete failed, check the id and try again.");
					}
				} catch (DAOException e) {
					throw new RuntimeException(e);
				} catch (NumberFormatException e) {
					player.sendStringMessage("%internal%" + ChatColor.RED + "You must input a number.");
				}
			}
			return true;
			...
			try (IContext ctx = tregmine.createContext()) {
				...
				if (messages.size() == 0) {
					player.sendStringMessage("%internal%" + ChatColor.AQUA + "You haven't received any messages.");
				} else if (messages.size() == 1) {
					player.sendStringMessage(
							"%internal%" + ChatColor.AQUA + "You have " + messages.size() + " message.");
				} else {
					player.sendStringMessage(
							"%internal%" + ChatColor.AQUA + "You have " + messages.size() + " messages.");
				}
			} catch (DAOException e) {
				throw new RuntimeException(e);
			}
			...
			if (player.getIsStaff()) {
				if (getArgs(args) < 2) {
					player.sendStringMessage("%internal%" + ChatColor.RED + "You must provide a player");
				} else {
					try (IContext ctx = tregmine.createContext()) {
						IMailDAO mail = ctx.getMailDAO();
						...
						player.sendStringMessage("%internal%" + ChatColor.AQUA + args[1] + " has received " + amount
				...
				try (IContext ctx = tregmine.createContext()) {
					...
					player.sendStringMessage("%internal%" + ChatColor.AQUA + "You have received " + amount + " "
			...
			try (IContext ctx = tregmine.createContext()) {
				...
				for (String[] message : messages) {
					player.sendStringMessage("%internal%" + ChatColor.AQUA + "You have a message from " + message[0]
							+ " [ID " + message[4] + "]");
					player.sendStringMessage("%internal%" + ChatColor.AQUA + "\"" + message[3].trim() + "\"");
				}
				if (messages.size() == 0) {
					player.sendStringMessage("%internal%" + ChatColor.AQUA + "You haven't received any messages.");
				}
	...
	public void sendHelp(TregminePlayer player) {
		...
		help[1] = "%internal%" + ChatColor.AQUA + "To send a message, type " + ChatColor.GRAY
				+ "/mail send <player> <message>";
		help[2] = "%internal%" + ChatColor.AQUA + "To delete a message, type " + ChatColor.GRAY
				+ "/mail delete <mail_id>";
		...
		if (player.getIsStaff()) {
			help[5] = "%internal%" + ChatColor.AQUA
					+ "To get the total messages you or another player has received, type " + ChatColor.GRAY
					+ "/mail tbt <player>";
		} else {
			help[5] = "%internal%" + ChatColor.AQUA + "To get the total messages you have received, type "
