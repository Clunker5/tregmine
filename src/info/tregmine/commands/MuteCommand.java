...
public class MuteCommand extends AbstractCommand{
	...
	public boolean handlePlayer(TregminePlayer player, String[] args){
		...
		if(!mutee.getRank().canBeMuted()){
			player.sendSpigotMessage(new TextComponent(ChatColor.RED + "You cannot mute "), mutee.decideVS(player), new TextComponent(ChatColor.RED + " because their rank bypasses muting."));
			return true;
		}
		int duration;
		try{
			...
			player.sendStringMessage(ChatColor.YELLOW + "Invalid duration specified; /mute <player> <duration:-1 for indefinite>");
			return true;
		}
		...
		player.sendSpigotMessage(mutee.getChatName(), new TextComponent(ChatColor.YELLOW + " has been muted for" + suffix));
		return true;
	}
}
