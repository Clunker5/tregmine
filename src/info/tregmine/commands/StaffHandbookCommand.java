...
public class StaffHandbookCommand extends AbstractCommand{
	...
	public boolean handlePlayer(TregminePlayer a, String[] b){
		...
		try (IContext ctx = tregmine.createContext()) {
            ...
            a.sendStringMessage(ChatColor.GOLD + t.getConfig().getString("general.servername") + ChatColor.AQUA + " Staff Handbook");
            for(String[] rule : handbook){
            	a.sendStringMessage(ChatColor.translateAlternateColorCodes('#', rule[0]) + ChatColor.RESET + ". " + ChatColor.translateAlternateColorCodes('#', rule[1]));
            }
