...
public class DiscordCommand extends AbstractCommand{
			...
			try{
				...
				if(validCode){
					try(IContext ctx = this.tregmine.getContextFactory().createContext()){
						...
						if(linked != -1){
							sender.sendStringMessage(ChatColor.RED + "Already linked to `" + ChatColor.ITALIC + usernameFor(linked) + ChatColor.RESET + ChatColor.RED + "`");
							return true;
						}
						long dID = this.tregmine.getDiscordSRV().getDiscordIDAndDeregisterCode(codeSubmitted);
						boolean success = ddao.link(sender, dID);
						if(success){
							sender.sendStringMessage(ChatColor.GREEN + "Successfully linked to `" + ChatColor.ITALIC + usernameFor(dID) + ChatColor.RESET + ChatColor.GREEN + "`");
							this.tregmine.getDiscordSRV().getAPI().getUserById(dID + "").getPrivateChannel().sendMessage("Successfully linked to `" + sender.getName() + "`");
						}else{
							sender.sendStringMessage(ChatColor.RED + "Failed to link to `" + ChatColor.ITALIC + usernameFor(dID) + ChatColor.RESET + ChatColor.RED + "`. Are you already linked?");
						}
		...
		case "unlink":
			try(IContext ctx = this.tregmine.getContextFactory().createContext()){
				...
				if(userid == -1){
					...
					usr.getPrivateChannel().sendMessage("You've been unlinked from `" + sender.getName() + "` - You can now link to another user.");
					sender.sendStringMessage(ChatColor.GREEN + "You've been unlinked from `" + usr.getName() + "`. You can now link to another user.");
	...
	private void linkPlayer(TregminePlayer s, String discordID){
		try(IContext ctx = this.tregmine.getContextFactory().createContext()){
			...
			if(linked != -1){
				s.sendStringMessage(ChatColor.RED + "Already linked to `" + ChatColor.ITALIC + usernameFor(linked) + ChatColor.RESET + ChatColor.RED + "`");
				return;
			}else{
			}
		} catch (DAOException e) {
