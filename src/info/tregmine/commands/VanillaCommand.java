...
public class VanillaCommand extends AbstractCommand{
	...
	public boolean handlePlayer(TregminePlayer sender, String[] args){
		...
		if(sender.getWorld() == plugin.getServer().getWorld("world")){
				sender.gotoWorld(sender.getPlayer(), plugin.getServer().getWorld("vanilla").getSpawnLocation(), ChatColor.YELLOW + "Thanks for riding the Starlight Express!", ChatColor.RED + "The Starlight Express is having some issues, try again later.");
