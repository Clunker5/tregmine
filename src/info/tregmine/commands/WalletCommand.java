...
public class WalletCommand extends AbstractCommand
{
    ...
    private boolean balance(TregminePlayer player)
    {
        try (IContext ctx = tregmine.createContext()) {
            ...
            if (balance >= 0) {
                player.sendStringMessage("You have " + GOLD + FORMAT.format(balance)
                        + WHITE + " Tregs.");
            }
            else {
        ...
        try (IContext ctx = tregmine.createContext()) {
            ...
            if (balance >= 0) {
                ...
                target.sendStringMessage(player.getName() + AQUA +
                    ...
                    " Tregs.");
                player.sendStringMessage(" You have " + GOLD +
            ...
            int amount)
    {
        ...
        try (IContext ctx = tregmine.createContext()) {
            ...
            if (walletDAO.take(player, amount)) {
                ...
                player.sendStringMessage(AQUA + "You donated to "
                        + target.getName() + " " + GOLD
                        + FORMAT.format(amount) + AQUA + " Tregs.");
                target.sendStringMessage(AQUA + "You received " + GOLD
            ...
            int amount)
    {
        ...
        try (IContext ctx = tregmine.createContext()) {
            ...
            if (walletDAO.take(player, amount)) {
                ...
                player.sendStringMessage(AQUA + "You gave " + target.getName()
