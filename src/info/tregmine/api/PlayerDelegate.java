...
public abstract class PlayerDelegate
{
    ...
    public void sendMessage(BaseComponent p0) {
        checkState();
        delegate.spigot().sendMessage(p0);
    }

    public void sendMessage(Object ...p0) {
        checkState();
        BaseComponent[] baseComponents = new BaseComponent[p0.length];
        for (int i = 0; i < baseComponents.length; i++) {
            Object object = p0[i];
            if (object instanceof String) {
                baseComponents[i] = new TextComponent((String) object);
            } else if (object instanceof BaseComponent) {
                baseComponents[i] = (BaseComponent) object;
            }
        }
        delegate.spigot().sendMessage(baseComponents);
    }

    public void sendError(String message) {
        checkState();
        delegate.spigot().sendMessage(ChatMessageType.ACTION_BAR, new ComponentBuilder(message).color(net.md_5.bungee.api.ChatColor.RED).create());
    }
    ...
    public void sendMessage(ChatMessageType p0, BaseComponent p1) {
        checkState();
        delegate.spigot().sendMessage(p0, p1);
    }
    ...
    public void setBanned(boolean p0)
    {
        checkState();
        if (p0) {
            delegate.getServer().getBanList(BanList.Type.NAME).addBan(delegate.getName(), ChatColor.RED + "You are banned from Tregmine.", null, "Administrator");
        } else {
            delegate.getServer().getBanList(BanList.Type.NAME).pardon(delegate.getName());
        }
    }
