...
public class DBDiscordDAO implements IDiscordDAO {
	...
	public long isLinked(TregminePlayer s) {
		try {
			java.sql.PreparedStatement stmt = this.conn.prepareStatement("SELECT discord_id FROM player_discord WHERE player_id = ?");
	...
	public boolean link(TregminePlayer s, long discordID) {
		if(isLinked(s) != -1) return false;
		if(discordID == -1) return false;
		try{
			PreparedStatement stmt = this.conn.prepareStatement("INSERT INTO player_discord (player_id, discord_id) VALUES (?, ?)");
			stmt.setInt(1, s.getId());
