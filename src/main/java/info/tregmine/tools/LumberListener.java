...
public class LumberListener implements Listener {
    ...
    public List<Block> anyConnectedBlocks(Block block, List<Block> cBlock) {
        ...
        for (Block b : knownBlocks) {
            ...
            if (eleven.getType().equals(material) && !cBlock.contains(eleven))
                cBlock.add(eleven);
            if (twelve.getType().equals(material) && !cBlock.contains(twelve))
                cBlock.add(twelve);
            if (thirteen.getType().equals(material) && !cBlock.contains(thirteen))
                cBlock.add(thirteen);
            if (fourteen.getType().equals(material) && !cBlock.contains(fourteen))
                cBlock.add(fourteen);

            if (fifthteen.getType().equals(material) && !cBlock.contains(fifthteen))
                cBlock.add(fifthteen);
            if (sixteen.getType().equals(material) && !cBlock.contains(sixteen))
                cBlock.add(sixteen);
            if (eighteen.getType().equals(material) && !cBlock.contains(eighteen))
                cBlock.add(eighteen);
            if (nineteen.getType().equals(material) && !cBlock.contains(nineteen))
                cBlock.add(nineteen);
        }
    ...
    public void Lumber(BlockBreakEvent event) {
        ...
        for (Block b : connectedBlocks) {
            ...
            byte data = 0;
            if (b.getData() == (byte) 0 || b.getData() == (byte) 4 || b.getData() == (byte) 8
                    || b.getData() == (byte) 12) {
                data = 0;
            } else if (b.getData() == (byte) 1 || b.getData() == (byte) 5 || b.getData() == (byte) 9
                    || b.getData() == (byte) 13) {
                data = 1;
            } else if (b.getData() == (byte) 2 || b.getData() == (byte) 6 || b.getData() == (byte) 10
                    || b.getData() == (byte) 14) {
                data = 2;
            } else if (b.getData() == (byte) 3 || b.getData() == (byte) 7 || b.getData() == (byte) 11
                    || b.getData() == (byte) 15) {
                data = 3;
            } else {
                data = 0;
