...
public class DBZonesDAO implements IZonesDAO {
    ...
    public void addRectangle(int zoneId, Rectangle rect) throws DAOException {
        String sql = "INSERT INTO zone_rect (zone_id, rect_x1, rect_y1, " + "rect_x2, rect_y2) VALUES (?, ?, ?, ?, ?)";
    ...
    @Override
    public void addUser(int zoneId, int userId, Zone.Permission perm) throws DAOException {
    ...
    public List<Lot> getLots(String world) throws DAOException {
        String sql = "SELECT zone_lot.* FROM zone_lot " + "INNER JOIN zone USING (zone_id) WHERE zone_world = ?";
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = stmt.getResultSet()) {
                while (rs.next()) {
                    ...
                    Zone.Permission permission = Zone.Permission.fromString(rs.getString("user_perm"));
    ...
    public void updateZone(Zone zone) throws DAOException {
        String sql = "UPDATE zone SET zone_world = ?, zone_name = ?, "
                ...
                + "zone_communist = ?, zone_publicprofile = ?, " + "zone_entermessage = ?, zone_exitmessage = ?, "
                + "zone_flags = ? " + "WHERE zone_id = ?";
