...
public class DBBankDAOOLD implements IBankDAO {
    private Connection conn;
    private Random r;

    public DBBankDAOOLD(Connection conn) {
        this.conn = conn;
        this.r = new Random();
    }
    ...
    public void deposit(Bank bank, Account acct, int playerId, long amount) throws DAOException {
        String sqlAcc = "UPDATE bank_account SET " + "account_balance = account_balance + ? "
        ...
        String sqlTransaction = "INSERT INTO bank_transaction(account_id, " + "player_id, transaction_type, "
    ...
    private int getMaxAccountNr(Bank bank) throws DAOException {
        String sql = "SELECT coalesce(max(account_number), 0) FROM bank_account " + "WHERE bank_id = ?";
        try (PreparedStatement stm = conn.prepareStatement(sql)) {
            stm.setInt(1, bank.getId());
            stm.execute();
    ...
    public void setPin(Account acct, String pin) throws DAOException {
        String sql = "UPDATE bank_account SET account_pin = ? " + "WHERE account_number = ?";
        try (PreparedStatement stm = conn.prepareStatement(sql)) {
            stm.setString(1, pin);
