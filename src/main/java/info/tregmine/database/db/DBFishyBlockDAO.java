...
public class DBFishyBlockDAO implements IFishyBlockDAO {
	...
	public void insert(FishyBlock block) throws DAOException {
		String sql = "INSERT INTO fishyblock (player_id, fishyblock_created, "
				...
				+ "fishyblock_signx, fishyblock_signy, fishyblock_signz, " + "fishyblock_storedenchants) ";
		...
		try (PreparedStatement stmt = conn.prepareStatement(sql)) {
			...
			try (ResultSet rs = stmt.getResultSet()) {
				while (rs.next()) {
					...
					boolean storedEnchants = "1".equals(rs.getString("fishyblock_storedenchants"));

					World world = getWorld(server, worldName);
					Location blockLoc = new Location(world, blockX, blockY, blockZ);
	...
	public void update(FishyBlock block) throws DAOException {
		String sql = "UPDATE fishyblock SET player_id = ?, " + "fishyblock_material = ?, fishyblock_data = ?, "
				+ "fishyblock_enchantments = ?, fishyblock_cost = ?, "
				+ "fishyblock_inventory = ?, fishyblock_world = ?, " + "fishyblock_blockx = ?, fishyblock_blocky = ?, "
				+ "fishyblock_blockz = ?, fishyblock_signx = ?, " + "fishyblock_signy = ?, fishyblock_signz = ?, "
