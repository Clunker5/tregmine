...
public class QuadTree<V>
{
  ...
  public static class Node<V>
  {
    ...
    public V find(Point p)
    {
      ...
      if (this.tl.contains(p)) {
        return (V)this.tl.find(p);
      }
      if (this.tr.contains(p)) {
        return (V)this.tr.find(p);
      }
      if (this.bl.contains(p)) {
        return (V)this.bl.find(p);
      }
      if (this.br.contains(p)) {
        return (V)this.br.find(p);
      }
      return null;
    }
      ...
      throws IntersectionException
    {
      ...
      if (!this.split)
      {
        ...
        for (Map.Entry<Rectangle, V> entry : this.values.entrySet())
        {
          Rectangle candidate = (Rectangle)entry.getKey();
    ...
    public boolean findIntersections(Rectangle rect)
    {
      ...
      if ((this.br.intersects(rect)) && (this.br.findIntersections(rect))) {
        return true;
      }
      return false;
    }
    ...
    throws Exception
  {
    ...
    float avgFindTime = 0.0F;float avgLinearSearchTime = 0.0F;
    for (Map.Entry<Rectangle, Integer> entry : values.entrySet())
    {
      ...
      long t = System.nanoTime();
      Integer cmp = (Integer)tree.find(p);
      t = System.nanoTime() - t;
