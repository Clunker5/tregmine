...
public class TargetBlock {
     ...
     * @param maxDistance    How far it checks for blocks
     * @param checkDistance  How often to check for blocks, the smaller the more precise
     * @param blocksToIgnore String ArrayList of what block ids to ignore while checking
     *                       for viable targets
     */
     ...
     * @param maxDistance    How far it checks for blocks
     * @param checkDistance  How often to check for blocks, the smaller the more precise
     * @param blocksToIgnore Array of what block ids to ignore while checking for viable
     *                       targets
     */
     ...
     * @param maxDistance    How far it checks for blocks
     * @param checkDistance  How often to check for blocks, the smaller the more precise
     * @param blocksToIgnore Integer array of what block ids to ignore while checking for
     *                       viable targets
     */
    ...
    private boolean blockToIgnoreHasValue(Material block) {
    ...
    private Material[] convertStringArraytoMaterialArray(ArrayList<String> array) {
        if (array != null) {
            Material materialarray[] = new Material[array.size()];
            String[] stringArray = (String[]) array.toArray();
            for (int i = 0; i < array.size(); i++) {
                try {
                    materialarray[i] = Material.matchMaterial(stringArray[i]);
                } catch (NumberFormatException nfe) {
                    materialarray[i] = Material.AIR;
                }
            }
    ...
    public double getDistanceToBlock() {
        Vector blockUnderPlayer = new Vector((int) Math.floor(loc.getX() + 0.5), (int) Math.floor(loc.getY() - 0.5),
    ...
    public Block getFaceBlock() {
        while ((getNextBlock() != null) && ((getCurrentBlock().getType() == Material.AIR)
                || this.blockToIgnoreHasValue(getCurrentBlock().getType())))
            ;
    ...
    public Block getNextBlock() {
        ...
        do {
            ...
            targetPos = new Vector(targetPosDouble.getBlockX(), targetPosDouble.getBlockY(),
                    targetPosDouble.getBlockZ());
        } while (curDistance <= maxDistance && targetPos.getBlockX() == prevPos.getBlockX()
                && targetPos.getBlockY() == prevPos.getBlockY() && targetPos.getBlockZ() == prevPos.getBlockZ());
        if (curDistance > maxDistance) {
            return null;
        }
    ...
    public boolean setTargetBlock(String type) {
        ...
        if (mat != null) {
            ...
            while ((getNextBlock() != null) && ((getCurrentBlock().getType() == Material.AIR)
                ...
                ;
            if (getCurrentBlock() != null) {
                Block blk = loc.getWorld().getBlockAt(targetPos.getBlockX(), targetPos.getBlockY(),
                        targetPos.getBlockZ());
                blk.setType(mat);
                return true;
            }
     ...
     */
    private void setValues(Location loc, int maxDistance, double viewHeight, double checkDistance,
                           Material[] blocksToIgnore) {
        this.loc = loc;
