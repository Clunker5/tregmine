...
public class FishyBlockListener implements Listener {
    ...
    private void addEnchants(ItemStack stack, FishyBlock fishyBlock) {
        ...
        if (fishyBlock.hasStoredEnchantments()) {
            ...
            for (Map.Entry<Enchantment, Integer> enchant : enchants.entrySet()) {
                enchantMeta.addStoredEnchant(enchant.getKey(), enchant.getValue(), false);
            }
    ...
    private boolean compareEnchants(Map<Enchantment, Integer> fst, Map<Enchantment, Integer> snd) {
        boolean match = true;
        ...
        if (player.getChatState() == TregminePlayer.ChatState.FISHY_SETUP) {
            ...
            if ("changecost".equalsIgnoreCase(textSplit[0])) {
                ...
                try (IContext ctx = plugin.createContext()) {
                    IFishyBlockDAO fishyBlockDAO = ctx.getFishyBlockDAO();
                    fishyBlockDAO.update(fishyBlock);
                    fishyBlockDAO.insertTransaction(fishyBlock, player, TransactionType.WITHDRAW, added);
                } catch (DAOException e) {
                    throw new RuntimeException(e);
                }
            ...
            if ("buy".equalsIgnoreCase(textSplit[0])) {
                ...
                try (IContext dbCtx = plugin.createContext()) {
                    ...
                    if (walletDAO.take(player, cost)) {
                        ...
                        fishyBlockDAO.update(fishyBlock);
                        fishyBlockDAO.insertTransaction(fishyBlock, player, TransactionType.BUY, added);
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (fishyBlocks.containsKey(loc)) {
            ...
            if (fishyBlock.getPlayerId() == player.getId()
                ...
                if (fishyMaterial.equals(heldMaterial)) {
                    ...
                    if (fishyBlock.hasStoredEnchantments()) {
                        ...
                        if (storageMeta != null) {
                            match = compareEnchants(fishyBlock.getEnchantments(), storageMeta.getStoredEnchants());
                        } else if (fishyBlock.getEnchantments().size() > 0) {
                            match = false;
                        }
                    } else {
                        match = compareEnchants(fishyBlock.getEnchantments(), heldItem.getEnchantments());
                    }
                }
                ...
                if (match) {
                    ...
                    try (IContext ctx = plugin.createContext()) {
                        IFishyBlockDAO fishyBlockDAO = ctx.getFishyBlockDAO();
                        fishyBlockDAO.update(fishyBlock);
                        fishyBlockDAO.insertTransaction(fishyBlock, player, TransactionType.DEPOSIT, allAmount);
                    } catch (DAOException e) {
                        throw new RuntimeException(e);
                    }
                }
            ...
            if (heldItem.getType() == Material.RAW_FISH && newFishyBlock == null) {
                ...
                Location signLoc = new Location(loc.getWorld(), loc.getX() + face.getModX(),
                        loc.getY() + face.getModY(), loc.getZ() + face.getModZ());
            ...
            // This is when the player sets the type of the fishy block
            else if (newFishyBlock != null && loc.equals(newFishyBlock.getBlockLocation())
                ...
                if (enchantments == null || enchantments.size() == 0) {
                    if (heldItem.hasItemMeta()) {
                        EnchantmentStorageMeta enchantMeta = getStorageMeta(heldItem);
                        if (enchantMeta != null) {
