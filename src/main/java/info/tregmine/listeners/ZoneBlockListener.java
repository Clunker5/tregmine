...
public class ZoneBlockListener implements Listener {
    ...
    public void onBlockBreak(BlockBreakEvent event) {
		 ...
		 * 
		 * if (Distance.calc2d(a, b) > 50) { player.sendMessage(ChatColor.YELLOW
		 ...
		 * event.setCancelled(true); return; } } else {
    ...
    public void onBlockTrample(PlayerInteractEvent event) {
        ...
        // Check it's farmland (otherwise it would stop pressure plates and
        // such)
    ...
    public void onLiquid(BlockFromToEvent event) {
        ...
        if (zone == null && zoneTo != null) {
            event.setCancelled(true);
            return;
        }

        // If liquid flows from a zone into another zone
        if (zone != null && zoneTo != null && !zone.getName().equalsIgnoreCase(zoneTo.getName())) {
            event.setCancelled(true);
            return;
        }
        ...
        if (lot != null && lotTo != null && !lot.getName().equalsIgnoreCase(lotTo.getName())) {
            event.setCancelled(true);
            return;
        }
