	...
	public void buttons(PlayerInteractEvent event) {
		...
		if (event.getAction() == Action.LEFT_CLICK_BLOCK || event.getAction() == Action.RIGHT_CLICK_BLOCK) {
			if (event.getClickedBlock().getType().equals(Material.STONE_BUTTON)) {
				...
				final TregminePlayer player = plugin.getTregmine().getPlayer(event.getPlayer());
				NumberFormat format = NumberFormat.getNumberInstance();
				int coordX = block.getX();
				int coordY = block.getY();
				int coordZ = block.getZ();
				try (IContext ctx = plugin.getTregmine().createContext()) {
					...
					if (coordX == 599 && coordY == 20 && coordZ == -96) {
						if (wallet.take(player, 15000)) {
							plugin.getServer().broadcastMessage(
									ChatColor.YELLOW + "You got a bitchslap from " + player.getChatName());
							player.sendStringMessage(ChatColor.GREEN + "15,000 Tregs were removed from your wallet.");
						} else {
							player.sendStringMessage(ChatColor.RED + "You need " + format.format(difference)
									+ " more Tregs for this button. (15,000)");
						}
						...
						if (wallet.take(player, 15000)) {
							if (player.getName().equals("Camrenn")) {
								plugin.getServer().broadcastMessage(
										ChatColor.LIGHT_PURPLE + "You just got hugged by princess Camrenn!");
								plugin.getServer()
										.broadcastMessage(ChatColor.LIGHT_PURPLE + "(Be happy, but not too happy)");
							} else {
								plugin.getTregmine().broadcast(
										new TextComponent(ChatColor.LIGHT_PURPLE + "You got a hug from "),
										player.getChatName());
								player.sendStringMessage(
										ChatColor.GREEN + "15,000 Tregs were removed from your wallet.");
							}
						} else {
							player.sendStringMessage(ChatColor.RED + "You need " + format.format(difference)
									+ " more Tregs for this button. (15,000)");
						...
						if (player.getWorld().getBlockAt(598, 17, -108).getType().equals(Material.STATIONARY_LAVA)) {
							...
							scheduler.scheduleSyncDelayedTask(plugin, new Runnable() {
								...
								public void run() {
									for (int y = 20; y < 34; y++) {
										for (int z = -102; z > -106; z--) {
											player.getWorld().getBlockAt(614, y, z).setTypeId(Material.WATER.getId(),
													false);
										}
									}
								}
							...
							scheduler.scheduleSyncDelayedTask(plugin, new Runnable() {
								...
								public void run() {
									for (int y = 20; y < 34; y++) {
										for (int z = -102; z > -106; z--) {
											player.getWorld().getBlockAt(614, y, z).setTypeId(Material.LAVA.getId(),
													false);
										}
									}
								}
						...
						if (wallet.take(player, 25000)) {
							...
							player.sendStringMessage(ChatColor.AQUA + "You received 1 coupon for 25,000 Tregs.");
						} else {
							player.sendStringMessage(ChatColor.RED + "You need at least 25,000 Tregs for this button!");
						}
					}
