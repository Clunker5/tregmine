	...
	public void run()
	{
		...
		for(Player player : Bukkit.getOnlinePlayers()){
			...
			if(dplayer.isAfk() && !dplayer.alertedAfk()){
				DiscordSRV.sendMessage(DiscordSRV.chatChannel, dplayer.getChatNameNoColor().replace("[AFK]", "") + " is now AFK.");
				dplayer.setAlerted(true);
			}else if(!dplayer.isAfk() && dplayer.alertedAfk()){
				DiscordSRV.sendMessage(DiscordSRV.chatChannel, dplayer.getChatNameNoColor().replace("[AFK]", "") + " is no longer AFK.");
