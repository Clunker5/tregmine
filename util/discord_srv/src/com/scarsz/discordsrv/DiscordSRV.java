...
public class DiscordSRV extends JavaPlugin {
	...
	public void onEnable() {
		...
		if (tregmine == null) {
			...
			if (mainPlugin != null) {
				...
				Tregmine.LOGGER.info(getDescription().getName() + " " + getDescription().getVersion()
		...
		if (getConfig().getDouble("ConfigVersion") < Double.parseDouble(getDescription().getVersion())
				|| !getConfig().isSet("ConfigVersion"))
			try {
				// Files.move(new File(getDataFolder(), "config.yml"), new
				// File(getDataFolder(), "config.yml-build." +
				// getConfig().getDouble("ConfigVersion") + ".old"));
				getLogger().info("Your DiscordSRV config file was outdated; attempting migration...");

				File config = new File(getDataFolder(), "config.yml");
				File oldConfig = new File(getDataFolder(),
						"config.yml-build." + getConfig().getDouble("ConfigVersion") + ".old");
				...
				for (String line : newConfigLines) {
					...
					for (String segment : line.split(": +|:"))
						lineSplit.add(segment);
					if (lineSplit.size() >= 2)
						newConfigMap.put(lineSplit.get(0), lineSplit.get(1));
				}

				for (String key : oldConfigMap.keySet()) {
					if (newConfigMap.containsKey(key) && !key.startsWith("ConfigVersion")) {
						getLogger().info("Migrating config option " + key + " with value " + oldConfigMap.get(key)
								+ " to new config");
				...
				for (String line : newConfigLines) {
					if (line.startsWith("#") || line.startsWith("ConfigVersion"))
						continue;
				...
				getLogger()
						.info("Migration complete. Note: migration does not apply to config options that are lists.");
				reloadConfig();
		...
		if (!plugin.getConfig().getBoolean("UpdateCheckDisabled")) {
			double latest = Double
					.parseDouble(requestHttp("https://raw.githubusercontent.com/Scarsz/DiscordSRV/master/latestbuild"));
			if (latest > Double.parseDouble(getDescription().getVersion())) {
				getLogger().warning(System.lineSeparator() + System.lineSeparator()
						+ "The current build of DiscordSRV is outdated! Get build " + latest
						+ " at http://dev.bukkit.org/bukkit-plugins/discordsrv/" + System.lineSeparator()
						+ System.lineSeparator());
				updateIsAvailable = true;
			}

			double minimum = Double.parseDouble(
					requestHttp("https://raw.githubusercontent.com/Scarsz/DiscordSRV/master/minimumbuild"));
			if (minimum > Double.parseDouble(getDescription().getVersion())) {
				getLogger().warning(System.lineSeparator() + System.lineSeparator()
						+ "The current build of DiscordSRV does not meet the minimum! DiscordSRV will not start. Get build "
						+ latest + " at http://dev.bukkit.org/server-mods/discordsrv/" + System.lineSeparator()
						+ System.lineSeparator());
		...
		if (guild == null) {
			getLogger().severe(System.lineSeparator() + System.lineSeparator()
					+ "DiscordSRV could not find the server ID you specified- set it in the config. Plugin being disabled."
					+ System.lineSeparator() + System.lineSeparator());
			Bukkit.getPluginManager().disablePlugin(this);
			return;
		}
		if (chatChannel == null)
			getLogger().warning("Specified chat channel ID from config could not be found");
		if (consoleChannel == null)
			getLogger().warning("Specified console channel ID from config could not be found");
		...
		// in-game chat events
		getServer().getPluginManager().registerEvents(new ChatListener(api, this.tregmine, this), this);
		getServer().getPluginManager().registerEvents(new PlayerDeathListener(api, tregmine), this);
		...
		if (channelTopicUpdater == null) {
			channelTopicUpdater = new ChannelTopicUpdater(api, tregmine, this);
			channelTopicUpdater.start();
		}

		// player join/leave message events
		if (getConfig().getBoolean("MinecraftPlayerJoinMessageEnabled"))
			getServer().getPluginManager().registerEvents(new PlayerJoinLeaveListener(api, this.tregmine), this);
		...
		// - chat channel
		if (getConfig().getBoolean("DiscordChatChannelDiscordToMinecraft")
				|| getConfig().getBoolean("DiscordChatChannelMinecraftToDiscord")) {
			if (!testChannel(chatChannel))
				getLogger().warning("Channel \"" + chatChannel + "\" was not accessible");
			if (testChannel(chatChannel) && !chatChannel.checkPermission(api.getSelfInfo(), Permission.MESSAGE_WRITE))
				getLogger().warning("The bot does not have access to send messages in " + chatChannel.getName());
			if (testChannel(chatChannel) && !chatChannel.checkPermission(api.getSelfInfo(), Permission.MESSAGE_READ))
				getLogger().warning("The bot does not have access to read messages in " + chatChannel.getName());
		}
		// - console channel
		if (consoleChannel != null) {
			if (!testChannel(consoleChannel))
				getLogger().warning("Channel \"" + consoleChannel + "\" was not accessible");
			if (testChannel(consoleChannel)
					&& !consoleChannel.checkPermission(api.getSelfInfo(), Permission.MESSAGE_WRITE))
				getLogger().warning("The bot does not have access to send messages in " + consoleChannel.getName());
			if (testChannel(consoleChannel)
					&& !consoleChannel.checkPermission(api.getSelfInfo(), Permission.MESSAGE_READ))
				getLogger().warning("The bot does not have access to read messages in " + consoleChannel.getName());
		}
	...
	public void onDisable() {
		...
		if (chatChannel != null && getConfig().getBoolean("DiscordChatChannelServerShutdownMessageEnabled"))
			chatChannel.sendMessage(getConfig().getString("DiscordChatChannelServerShutdownMessage"));

		// disconnect from discord
		try {
			api.shutdown(false);
		} catch (Exception e) {
			getLogger().info("Discord shutting down before logged in");
		}
		api = null;

		// save unsubscribed users
		if (new File(getDataFolder(), "unsubscribed.txt").exists())
			new File(getDataFolder(), "unsubscribed.txt").delete();
		String players = "";
		...
		if (args[0].equalsIgnoreCase("setpicture")) {
			...
			try {
				api.getAccountManager()
						.setAvatar(AvatarUtil.getAvatar(new File(getDataFolder().getAbsolutePath() + "/picture.jpg")))
						.update();
	...
	private void buildJda() {
		...
		try {
			api = new JDABuilder().setBotToken(getConfig().getString("BotToken"))
					.addListener(new DiscordListener(getServer(), this)).setAutoReconnect(true).setAudioEnabled(false)
					.buildBlocking();
		} catch (LoginException | IllegalArgumentException | InterruptedException e) {
			getLogger().severe(System.lineSeparator() + System.lineSeparator() + "Error building DiscordSRV: "
	...
	public static void sendMessage(TextChannel channel, String message) {
		...
		new Thread(() -> {
			...
			if (api == null || channel == null || (!channel.checkPermission(api.getSelfInfo(), Permission.MESSAGE_READ)
					|| !channel.checkPermission(api.getSelfInfo(), Permission.MESSAGE_WRITE)))
				return;
			newMessage = ChatColor.stripColor(newMessage).replace("[m", "")
					.replaceAll("\\[[0-9]{1,2};[0-9]{1,2};[0-9]{1,2}m", "").replaceAll("\\[[0-9]{1,3}m", "")
					.replace("[m", "").replaceAll("\\[[0-9]{1,2};[0-9]{1,2};[0-9]{1,2}m", "")
			...
			if (newMessage.length() > 2000) {
				plugin.getLogger().warning("Tried sending message with length of " + newMessage.length() + " ("
						+ (newMessage.length() - 2000) + " over limit)");
	...
	public String getPrimaryGroup(Player player) {
		...
		TregminePlayer sender = tregmine.getPlayer(player);
		...
		// service =
		// DiscordSRV.plugin.getServer().getServicesManager().getRegistration(net.milkbowl.vault.permission.Permission.class);
		//
	...
	public static String getAllRoles(MessageReceivedEvent event) {
		...
		if (!roles.isEmpty())
			roles = roles.substring(0, roles.length()
					- DiscordSRV.plugin.getConfig().getString("DiscordToMinecraftAllRolesSeperator").length());
		return roles;
	}
